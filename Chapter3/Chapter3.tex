...
La base de données est accompagnée d'une interface web et un serveur Java J2EE.
...
Chaque peptide et chaque monomère y possèdent sa page dédiée.
...
Ces contributions nous ont d'ailleurs plusieurs fois menés à réaliser des changements au sein de l'ontologie de Norine et à la normalisation de concepts déjà présents.
...
Dans MyNorine, nous proposons à l'utilisateur d'exécuter s2m avant d'entrer son annotation de manière complétement manuelle.
...
Nous ajoutons comme filtre l'impossibilité d'être linéaire (nombre de liaisons supérieur ou égal à nombre de nœuds) et l'obligation de posséder au moins deux monomères ne faisant pas partie des 20 de base.
...
Les NRP étant une source très riche d'antibiotiques, de nombreuses équipes essayent de créer, à partir de NRP, de nouvelles molécules contres lesquelles les micro-organismes ne présentent pas de résistance.
...
Le Graal de la biologie de synthèse appliquée aux NRP est de savoir comment produire nos propres assemblages monomériques en manipulant des gènes NRPS.
...
En ajoutant des gènes d'enzymes de modification NRP au cluster de gène ou en en modifiant des déjà présentes, certaines équipes parviennent à l'ajout de nouveaux monomères ou la modification de certains monomères déjà inclus~\cite{giessen_ribosome-independent_2012}.
...
\begin{figure}[h!]
  \begin{center}
    ...
    L'expression de ce gène est modifiée et le domaine A produit ne correspond plus à un domaine A vert mais à un variant proche.
    Le monomère capturé par ce domaine ne correspond plus à un monomère vert mais directement au variant proche correspondant au domaine.}
  \end{center}
\end{figure}

La seconde possibilité de modification de NRP passe par la modification des synthétases.
Plus exactement par la modification du code génétique correspondant à un domaine A d'une synthétase.
...
En modifiant certains acides aminés composant un domaine A, la spécificité de ce domaine peut changer.
...
\begin{figure}[h!]
  \begin{center}
    ...
    \caption{\label{echange}La sous partie de gène NRPS correspondant au domaine A incluant un monomère vert est remplacé par un morceau de gène correspondant à un monomère bleu.
    En conséquence, le monomère inclus par la synthètase est bleu.}
  \end{center}
\end{figure}
...
Bien que les contraintes structurelles soient des contraintes fortes, certaines équipes ont réussit des preuves de concepts de cette technique.
Par exemple, les auteurs de \cite{kries_subdomain_2015} procèdent à l'échange d'une sous partie de domaine A afin de modifier sa partie spécifique.
Ils ont ainsi réussi à modifier le domaine A du premier module de la Gramicidine S tout en gardant une production peptidique chimérique.
...
Les techniques que nous avons présenté ici sont toutes très encourageantes pour l'avenir de la biologie de synthèse pour les NRP.
...
Cependant il reste plusieurs grand obstacles à surmonter pour parvenir à la création d'antibiotiques NRP synthétiques.
Les différentes preuves de concept doivent être répliquées pour la fabrication de nombreux et divers NRP.
C'est en multipliant les expérieces que nous tirerons de l'information de ce qui fonctionne ou non.
De plus, il faudra outrepasser les difficultés de quantité de production.
Les divers NRP synthétiques créés sont créés avec des débits bien plus faibles que les NRP originaux.
...
C'est dans cette optique que j'ai créé un prototype pour la recommandation de cluster proches.
...
\begin{figure}[h!]
  \begin{center}
    ...
    Il doit chercher au sein d'une base de NRPS existants, ceux qui sont le plus proche de ce NRP et classer les meilleurs résultats dans l'ordre de ressemblance.
    Ces clusters proches devraient pouvoir aider les biologistes à choisir des NRPS suffisament proche du NRP souhaité pour pouvoir les dériver le plus facilement possible.}
  \end{center}
\end{figure}
...
Cependant, il possible que le contributeur ait entré directement les résultats d'une prédiction sans ajouter d'annotations car celles-ci suffiaient pour ses travaux.
...
Pour augmenter nombre, il sera nécessaire de valider d'autres structures manuellement.
...
Cette diversité de structures des fichiers d'annotation vient empécher la gestion automatique d'un grand nombre d'entrées.
Suite au retrait des annotations impossible à traiter dans un script (à moins de leur consacrer une excpetion), environ la moitié des annotations des NRP de Norine ne sont plus présentes.
