...
\begin{figure}[!ht]
  \begin{center}
    ...
    \caption{\label{markov}En vert, les chemins prits par la programation dynamique pour finalement obtenir le chemin optimal vers le motif a-b-d.}
  \end{center}
\end{figure}
...
Les débuts de la chaîne sera optimisée de manière exact jusqu'à une taille $k$ (préalablement définie par l'utilisateur), puis le reste des extensions se fera en utilisant l'algorithme glouton.
...
liste à chaque tour de boucle ($O(n)$). Contrairement à la résolution exacte présentée précedemment, si la fonction de comparaison
pour le tri ne dépend pas de $n$ cet algorithme est polynomial.
...
Pour cela, nous avons construit deux jeux de données de référence, au sein desquels les annotations attendues sont déjà connues.
...
Dans notre cas, une annotation est un FP si tous ses atomes sont couverts par l'annotation mais que celle-ci diffère du jeu référence.
...
Dans notre cas, les annotations de référence sont correctes, ce n'est pas possible.
Il est toujours possible d'obtenir une annotation couvrante vu qu'elles sont toutes présentes dans le jeu de référence.
\end{itemize}
