...
\paragraph{}Une fois assemblée, une protéine se replie sur elle même.
Les caractéristiques structurelles et physico-chimiques qui découlent de ce repliement, lui donnent son activité.
...
Ces propriétés sont donc dépendantes du repliement et des types de radicaux exposés.
...
Ici, la protéine synthétisée n'est pas le produit final mais une \textit{enzyme modulaire} agissant seule ou en complexe afin d'assembler les NRP.
...
\paragraph{}Enfin, le dernier type de liaison implique au moins un monomère de la catégorie des sucres (Glucose ou Arabinose par exemple).
Les sucres inclus dans les NRP connus sont toujours cyclisés.
Ils se lient par perte d'un groupement OH présent sur un carbone voisin de l'oxygène du cycle (voir figure \ref{ose}).
...
Beaucoup de NRP recensés sont des antibiotiques.
...
\paragraph{}Comme nous l'avons vu dans l'introduction précédente, les synthétases assemblant les NRP sont des protéines issues d'une synthèse classique.
Chaque NRPS est un complexe enzymatique extrêmement grand dont la dimension est parfois à comparer à un ribosome (TODO : exemple taille en aa).
Cependant, contrairement à un ribosome, les NRPS sont spécialisés.
Un NRPS ne peut produire qu'un seul type de NRP.
Elles sont organisées en modules consécutifs qui capturent modifient et intègrent chacun un monomère au NRP\cite{schwarzer_nonribosomal_2003,marahiel_modular_1997}.
...
Un module standard est d'abord composé d'un domaine de condensation (C) effectuant la liaison du peptide déjà formé au monomère en cours d'inclusion puis d'un module d'adénilation (A) permettant la capture et l'activation du monomère à insérer et d'un module de thiolation (T) effectuant la fixation du monomère capturé et le transfert du NRP depuis le domaine C précédent au domaine C suivant.
...
On peut également voir un domaine d'épimérisation (modification de l'acide aminé inclus) présent en fin de premier module ainsi qu'un module C supplémentaire en fin d'enzyme.
Ce dernier effectue la cyclisation du peptide.
...
Ils alignent un grand nombre de ces domaines pour en extraire les parties conservées entre domaines et les parties spécifiques au monomère à capturer.
...
Ces points conservés (a priori très importants pour la structuration du domaine), servent de repères vers des acides aminés proches faisant partie de la poche d'accueil du monomère.
