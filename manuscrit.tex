...
Ils sont la réplication d'un morceau d'ADN qui sera traduit par triplets de nucléotides en chaîne d'acides aminés.
Les 64 codons possibles ($4^3$ nucléotides) sont traduits en 20 acides aminés que l'on appelle les acides aminés protéinogènes (justement car ils interviennent dans la synthèse classique de protéines).
...
Sur le squelette est ancré une chaîne latérale variant d'un acide aminé à l'autre, leur donnant leurs spécificités (voir figure \ref{chaine_pep}).
...
\begin{figure}[h!]
  \begin{center}
    ...
    Au sein de de la Capreomycin IA, une troisième liaison est effectuée par le Dpr depuis son groupement amine de la chaîne latérale.}
  \end{center}
\end{figure}
...
Ainsi sur l'exemple (Voir figure \ref{DPR_incl}), on peut constater que le monomère nommé Dpr possède un groupement amine supplémentaire à celui déjà présent dans le squelette des acides aminés classiques.
Ce groupement en bout de chaîne latérale permet au Dpr de parfois se lier 3 fois et ainsi de casser la linéarité de la molécule assemblée.
...
L'efficacité des NRP produits peut quant à elle être expliquée par un effet de sélection.
...
Dans un second temps, le monomère est mené au site de condensation du début de module afin d'être inséré par le domaine C au début du NRP déjà formé.
...
Ils ne sont donc pas entourés de deux modules différents mais sont inclus respectivement dans le premier et dernier module.
...
Les domaines responsables de ces transformations sont appelés domaines d'épimérisation.
...
\paragraph{}Pour résumer, les enzymes de décoration post-synthèse sont capables d'effectuer 3 tâches que nous avons découvert sur l'exemple de la vancomycine :
\begin{itemize}
...
\paragraph{}Les PKS (Polyketyde Synthetase) sont, tout comme les NRPS, des enzymes modulaires permettant de synthétiser des polymères complexes par une voie non ribosomique.
...
\begin{figure}
  \begin{center}
    ...
    La longue chaîne carbonée de la première étape est créée par une répétition successive de 9 fois le même module.
...
\paragraph{}Les PKS et NRPS ont des origines communes dans l'évolution et il est logique de les retrouver aux mêmes endroits du génome.
Plusieurs NRP sont hybrides avec des PK, le cluster de gènes est alors composé de gènes NRPS et PKS.
...
Certains outils génériques comme CLUSEAN~\cite{weber_clusean:_2009} qui permettent de détecter des clusters de gènes quels qu'ils soient.
...
Tous ces outils pour dire que quel que soit le sous domaine des NRP étudiés, nous pouvons toujours trouver un outil spécialisé dans la recherche de leurs clusters de gènes.
...
Cette phylogénie a permis aux auteurs de diviser les domaines C en plusieurs groupes et ainsi de créer la première classification de ces domaines.
...
L'utilisation de HMM en faible nombre permet à ces logiciels d'être extrêmement rapides pour les recherches.
...
antiSMASH est sans doute le logiciel qui est venu révolutionner ces dernières années la prédiction de métabolites secondaires depuis l'ADN.
...
Par exemple, en lançant une analyse sur un génome bactérien, antiSMASH va commencer par rechercher les clusters de gènes avec CLUSEAN, puis identifier parmi les clusters détectés, ceux qui permettent la production de métabolites secondaires pour finir par lancer les outils spécifiques à chaque métabolite comme NRPSpredictor et NAPDOS sur les clusters NRPS.
...
Cette facilité d'utilisation et la puissance des outils utilisés dans le pipeline a permis un grand succès auprès de la communauté.
...
ClusterMine360 est une base de données recensant des clusters de gènes NRPS et PKS.
...
Ces prédictions de structures NRP sont donc à vérifier avant de les utiliser.
...
Les créateurs de la base cherchent la qualité avant la quantité.
...
Ce lien fort entre clusters de gènes et composés est très intéressant mais n'est entièrement disponible que pour 67 éléments.
...
Cependant, comme nous l'avons vu lors des descriptions de logiciels d'analyse de NRPS, cette compréhension ne permet pas encore d'avoir des résultats parfaits et complets.
...
L'analyse en résultant permet d'obtenir un spectre contenant différents pics représentant les masses des morceaux obtenus.
...
De plus, contrairement à l'annotation depuis l'ADN, nous sommes certains que la molécule existe puisqu'elle est directement tirée de l'environnement étudié.
...
Soit l'organisme est connu comme producteur d'un NRP dont on connaît la structure et l'annotation de NRPS peut être vérifiée.
Dans ce cas, la structure présente dans MIBiG est exacte même si on ne sait parfois pas quels gènes sont venus ajouter/modifier des morceaux du NRP.
...
\paragraph{}Beaucoup de NRP possèdent des activités très intéressantes (antibiotiques, antitumeurs, antidouleurs, ...).
À ce titre, nous cherchons en permanance à caractériser les activités de chaque nouvelle molécule découverte.
...
\section{Formalisation du problème d'annotaion}
...
Dans notre cas, nous n'avons pas choisi de démarer des clusters de gènes NRPS puisque des modèles déjà bien éprouvés existent déjà (NRPSpredictor, NaPDoS, ...) mais que leurs prédictions ne sont pas garanties.
Les avancées dans ce domaines viendront avec le nombre d'annotations fiables de binomes clusters/NRP qui permettrons l'amélioration des modèles d'apprentissage actuels.
...
Les deux logiciels sont oposés dans leur façon de prendre le problème.
CHUCKLES est constructif car il part des monomères pour recostituer le polymère alors que molBLOCKS est destructif car il part du polymère pour retrouver les monomères par découpes successives de liens.
Nous allons ici présenter ces deux approches.


\subsubsection{CHUCKLES, une méthode constructive}

\paragraph{}La technique appelée CHUCKLES, décrite en 1994, correspond parfaitement à ce que nous recherchons.
A des fins de requêtage et de compression, les auteurs avaient besoin d'un moyen de passer rapidement de la représentation monomérique à la représentation atomique et inversement.
La partie ``inverse'' (Back-Translation dans l'article) utilise la liste des 20 acides aminés standards (sans leurs atomes d'hydrogène) afin de les rechercher un à un dans la molécule à analyser.
Les acides aminés sont recherchés du plus gros au plus petit afin d'éviter des positionnements aléatoires des plus petits.
Les recherches des acides aminés peuvent être effectuées rapidement et de manière exactes grâce à de bonnes implémentations des SMARTS~\cite{jeliazkova_ambit-smarts:_2011}.
Cette technique fonctionne parfaitement avec les 20 acides aminés standards tant qu'ils ne sont pas transformés au sein de la molécule d'arrivée.
Dans le cas des peptides non-ribosomiques, le nombre de monomères s'élève à plus de 500 dont de nombreux très ressemblants entre eux.
Dès que ces monomères très ressemblants sont intégrés au NRP (et qu'ils ont été modifiés par les liaisons) il devient alors impossible en utilisant un algorithme simple, d'être certain de trouver la bonne structure monomérique.
Cette méthode est donc une bonne base de travail pour créer un outil d'annotation NRP mais n'est pas suffisant pour outrepasser la complexité des molécules que nous souhaitons traiter.

\paragraph{TODO : Schéma CHUCKLES}


\subsubsection{molBLOCKS, une méthode destructive}

\paragraph{}Une seconde méthode d'annotation cherche à ``détruire'' les liens pertinents de la molécule pour en extraire les composants.
Le logiciel molBLOCKS utilise cette stratégie.
Il découpe un ensemble de molécules d'intérêt selon les règles définies par l'utilisateur, jusqu'à ne plus trouver de liaisons ou atteindre une taille limite de fragments.
Le logiciel analyse ensuite la fréquence des fragments sur l'ensemble des molécules pour déterminer les fragments qui ont le moins de chance d'être artificiellement créés et ainsi les considérer comme les monomères réels.
Cette méthode à pour avantage de ne pas avoir besoin de base de donnée de monomères comme prérequis et propose une manière élégante de les retrouver.
Cependant rien ne garantit que les monomères créés par le logiciel ne soient ceux vraiment inclus biologiquement.
De plus, les monomères rares dans la base de données ne généreront pas de fragments avec une suffisamment grande redondance pour être considérés comme réels.
Enfin, si les règles de découpe sont mal choisies, les fragment générés n'auront aucune signification biologique.

\paragraph{TODO : Schéma molBLOCKS}
...
Nous possédons au sein de Norine, une grande base de connaissance de monomères existants (plus de 500) et nous pouvons nous appuyer sur cette base bien établie pour assurer l'existance des monomères lors des annotations.
...
Si d'un côté nous recherchons les monomères au sein du NRP puis de l'autre nous assemblons ceux trouvés, alors nous pouvons séparer notre programme en deux sous-problèmes complétement distincts.
...
En utilisant la base Norine, nous pouvons indépendament rechercher chacun des monomères connus au sein de la molécule à annoter.
...
Aucun des monmères choisis dans la solution ne peux être chevauchant avec un autre.
...
On appélera cette étape, l'étape de pavage.
...
\paragraph{Pavage des monomères}Une fois les monomères détectés, il est nécessaire de prendre une décision pour les
...
Pour cela, il est nécesaire de construire des jeux de données tests au sein desquels les annotations attendues seront déjà connues.
Ces bases de test doivent être composées de polymères comprenant leur SMILES, représentation de leur structure atomique ainsi que dde l'annotation attendue (un graphe monomérique).
...
s2m précalculera les indexes pour chaque base de monomère puis sera exécuté sur chaque polymère.
...
Parmis celles-ci se trouvent de nombreux petits polymères annotés ainsi que les monomères les constituant.
...
Les données de validation seront composées de deux jeux distrincts.
...
Les pourcentages de l'un à l'autre peuvent être fort différents dû à la grande variabilité du nombre d'atomes d'un monomère à l'autre.
...
\section{Les contributions de s2m à dans Norine}
