...
Parmis ces monomères, on compte les 20 acides aminés standards ainsi qu'un grand nombre de dérivés proches.
Après plusieurs modifications que nous détaillerons plus tard, il est par exemple possible d'obtenir des monomères méthylés ou oxydés (voir schéma TODO).
...
\paragraph{}Enfin, le dernier type de liaison que nous alons décrire implique au moins un monomère de la catégorie des sucres (Glucose ou Arabinose par exemple).
...
Ces logiciels sont extrémement rapide pour les recherches grâce à l'utilisation d'un faible nombre d'archétypes.
...
Les labels des noeuds font office de filtres de recherche et accélérent fortement la recherche.
...
Faisons une analogie avec un puzzle. La phase d'isomorphisme obtenient de nombreuses pièces de puzzle.
...
Le but était de découper une pizza (matrice), composée de cases de jambon (H dans la matrice) et de cases de sauce (T dans la matrice), en part dites ``royales'', tout en minimisant le gachis de pizza.
Les part royales sont des parts rectangulaires contenant au minimum 3 cases de jambon et représentant une surface max de 12 cases.

\paragraph{TODO : schéma pizzas}

\paragraph{}Ce problème a été prouvé NP-Complet sur le forum stackexchange\cite{de_biasi_complexity_2015} suite à la question d'un paticipant.
La preuve a été faite par réduction du problème ``Monotone cubic planar 1-3 SAT''.
Prouvons que le problème est NP-Complet par réduction du problème ``Pizza''.

\paragraph{}Premiérement, il est très facile de montrer par certificat que le problème de pavage est NP.
...
- Les atomes du peptide : Les atomes du peptide représenteront les cases de la matrice pizza.
- Les liens entre atomes : Les liens représentent les voisinage de deux cases.
Si deux cases sont voisine (voisinage de Manhattan), alors un lien existe entre les deux atomes qui les représentent.
- Les résidus : Les résidus correspondent aux parts royales de pizza.
...
On pourra toujours trouver au moins un tri de la liste qui atteingne la solution optimale.
...
Pour le précalcul, nous pouvons fixer la variable $k$ qui définiera la taille de chaine calculée avec le modèle markovien.
...
L'un des paramêtres est la limite max de nombre de tours.
...
L'influence de ces paramêtres sur l'exécution de l'algorithme sera étudiée durant la partie résultats.
