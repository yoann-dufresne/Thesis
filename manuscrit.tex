...
\chapter{Les peptides non ribosomiques et leur voie de synthèse alternative}
...
Par strucuture, il ne faut pas uniquement entendre comformation 2D ou 3D de la molécule mais également toutes les
proprétés physico-chimques qui résultent des atomes et liaisons présents.
...
de comparer sa structure avec des structures dont on connait déjà l'activité. Il existe beaucoup de techniques\{TODO Refs nécessaires\} de
comparaison mais elles ont toutes un point commun : Plus la comparaison de structure inclut de propriétés et plus
elle est précise mais difficile a effectuer. Le but est donc d'essayer d'étudier des propriétés pertienentes et
faciles à comparer.
...
églament nous appuyer sur l'information apportée par la structure monomérique de ces NRP. En 2012\cite{abdo_new_2012}
...
structure monomorérique au lieu de la structure atomique est la rapidité d'analyse. Là où une comparaison chimique
...
\paragraph{}Résumons la situation. Les NRP sont des molécules polymériques assemblées à partir d'un nombre de briques
de bases monomériques finies. Lors d'un séquencage de ces molécules, nous avons accès à la structure chimique de
ces composés mais pas toujours à leur structure monomérique. Cette structure nous intéressant particuliérement,
nous aimerions pouvoir remonter automatiquement depuis la structure chimique vers la structure monomorérique.
...
cette stratégie. Le logiciel découpe non pas une mais un ensemble de molécules d'intérêt selon les régles définies
...
qui ont le moins de chance d'être artificiellent créés et ainsi les considérer comme les monomères réels. Cette
...
De plus, les monomères rares dans la base de données ne généreront pas de fragments avec une suffisament grande
redondance pour être considérés comme réels. Enfin, si les régles de découpe sont mal choisies, les fragment générés
...
chimiques se représente immédiatement. Les atomes seront nos sommets de graphes et les liaisons les arêtes. Je rjouterai
comme petite subilité que les hydrogènes ne seront pas représentés comme sommets mais inclus directement dans le sommet
...
\paragraph{}Dès à présent dès que je parlerai de molécules quelles qu'elles soient, je parlerai par abus de language
...
\begin{definition}\textbf{Sous-graphe Maximum Commun :}
  ...
  est idententique au sommet
  de $V'$. Le Sous-graphe Maximum Commun est défini comme un graph $H(M, N)$ avec $M$ et $N$ construits comme suit. $\forall
...
Comme nous l'avons vu dans l'état de l'art, ce problème transformé en problème de graphe est un problème d'isomophisme de sous
graphe.
...
isomophismes de graphe ou de sous-graphe non labellés d'un graphe $G$ sur un graphe $H$.
...
\paragraph{}
  \begin{algorithm}[H]
    ...
    \KwResult{Un mapping entre les éléments de $V$ et une sous partie des éléments de $V'$ telle que $H$ soit un isomophisme de
    sous graphe de $H$}
	...
	$P \gets$ ensemble des $p(x,y)$ tq $x \in V$, $y \in V'$ et $\exist p(a,b) \in Mapping(s)$ tq $a$ et $x$ soient voisins dans
...
\begin{figure}
  ...
  \caption{\label{vf2}Représentation sous forme d'arbre d'une recherche de d'isomophisme de sous graphe par l'algorithme VF2}
\end{figure}

\paragraph{}Expliquons cet algorithme en utilisqnt le schéma \ref{vf2}.
...
'1'-'b'.
Récursivement l'algorithme conclut à un isomophisme {(1,b);(2,a);(3,c)}.

\paragraph{}En l'état l'algorithme VF2 ne permet de reconnaitre qu'un seul et unique isomophisme.
On peut modifier simplement l'algorithme afin de récupérer tous les isomophismes en ne retournant pas une fois que l'un d'eux est
trouvé mais en l'enregistrant dans un ensemble puis en continuant l'expliration de l'arbre jusqu'au bout.
...
Les labels des noeuds font office de filtes de recherche et permettent d'accélérer fortement la recherche.
...
figure \ref{representations}, première partie). Évidamment, les hydrogènes sont les atomes les plus représentés et ne filtrent
pas les recherches par eux-même. Cependant, ils sont un bon moyen de donner des précisions sur l'atome auquel ils sont liés.
...
difficultés à effectuer un isomophisme. En effet la lenteur qui peut venir de cet algorithme est dû au nombreux chemins qu'il
peut prendre lors de l'exploration.

\paragraph{}Cherchons encore à augmenter la sélectivité des labels en augmentant leur nombre. A partir d'un graph $G$ il est
possible de représenter ses relations d'adjacence au sein d'un second graph $L$. Pour cela, deux noeuds voisins de $G$ sont
...
gros avantage de cette représentation est d'inclure directement dans les labels des nouds ce qui touche à l'arité entre noeuds.
La fonction
...
Cependant cela pose plusieurs problèmes de répéter cette opération dans notre cas. Certains graphe vont diminuer de taille
jusqu'à disparaitre complétement (ce qui est peu pratique pour la comparaison) alors que d'autre vont grossir de plus en plus
...
on peut avoir plusieurs chaines représentant le même motif mais avec des ordres de parcours différents. Ce que nous appellerons
indexation sera donc la phase de recherche d'une chaine pour représenter chaque motif d'intérêt en essayant de minimiser le temps
que prendra la recherche de cette chaine. Pour la recherche de monomères dans
...
rechercher les noeuds du graphe le représentant. Ainsi on peut voir que quelque soit la chaine représentant le graphe, le même
...
s'attarderont à la recherche de chaines performantes (ie les plus rapide) lors la recherche d'un motif.


\subsubsection{Indexation gloutone}

\begin{figure}
  \includegraphics[width=300px]{Figures/s2m/indexation/chaine_fail.pdf}
  \caption{\label{chaine_fail}Deux exemple de chaine pour un motif simple}
\end{figure}

\paragraph{}Une bonne chaine est donc une chaine rapide mais a quoi ressemble une telle chaine ? Déterminons quel(s) critère(s)
simple(s) nous permettent de supposer qu'une chaine est bonne. Avec ce ou ces critères nous pourrons ensuite déterminer un
...
dessiné un motif simple suivi de deux chaines pouvant le représenter (lecture de l'ordre des chaines de gauche à droite). Pour
la première chaine, en partant du motif ne contenant qu'un A, on obtiendra 3 isomorphismes. En étendant ces chaines à deux A
voisins, on conserve toujours trois isomophismes puis deux en ajoutant B et enfin un seul avec le dernier A. Au total on aura
étendu 9 isomorphismes durant la recherche. En regardant maintenant la seconde chaine, on se rend compte que le nombre de ces
isomorphismes intermédiaires est bien plus faible grâce à la sélectivité du B recherché en tout premier (5 isomophismes
intermédiaires sont étendus). La différence décisive entre les deux chaines est donc le moment où le noeud avec un label sélectif
...
uniquemet le temps de calcul. Une fois cette base créée, nous pouvons estimer les fréquences d'apparition de chacun des labels
...
création de chaines par cette méthode est instantanée (<< 0.1s). Toutefois, rien ne garantit la qualité des chaines obtenues.
...
labels nécessaire sont également présent sur cette même figure. La chaine qui sera générée commence forcément par le noeud
labelé $a$ (fréquence la plus faible dans le jeu d'apprentissage) suivi de $b$ puis enfin $d$ (Le tout est résumé sur la figure
\ref{glouton}).

\begin{figure}
  \includegraphics[width=300px]{Figures/s2m/indexation/glouton.png}
  \caption{\label{glouton}En vert le chemin prit par l'algorithme glouton pour générer la chaine du motif a-b-d}
\end{figure}

\subsubsection{Indexation Markovienne}

\paragraph{}En utilisant la méthode gloutonne, nous obtenons rapidement les chaines de tous les motifs que nous
souhaitons indexer. Cependant, dans certains cas, une chaine générée de cette manière peut ne pas du tout être optimale.
Dès le tout début de la chaine il est possible que l'algorithme fasse des choix optimaux localement qui vont ensuite mener à un
...
\paragraph{}Afin d'évaluer la qualité d'une chaine, nous allons calculer son temps d'exécution sur notre base d'apprentissage.
Partons de la définition d'une chaine : Une chaine est composée d'une succession de noeuds. Ces noeuds vont lors de l'isomorphisme
être recherchés dans l'ordre. Pour rechercher une chaine de taille $n$ ($c_n$), il faut donc $n$ extensions successives. On
peut donc écrire que le temps de recherche d'une chaine est le temps cumulé de recherche de ses extensions :

\begin{equation}
 T(c_n) = T(p_0 \rightarrow p_1 \rightarrow ... \rightarrow p_n) = \sum_{i=1}^n T(p_{i-1} \rightarrow cp_i)
\end{equation}

Où $T()$ est la fonction représentant le temps de recherche d'une chaine, $p_i$ le un motif de taille $i$ et $p_{i-1} \rightarrow
p_i$ la dernière extension d'une chaine $c_i$ représentée par le passage du motif $p_{i-1}$ au motif $p_i$. Il est à noter que
rechercher une chaine de taille 1 revient à étendre un motif vide. C'est à dire qu'il est nécessaire de parcourir l'intégralité
...
\paragraph{TODO :}Figure arité restante.

\paragraph{}Lors d'une recherhe d'une chaine $c_{n-1}$, il est possible de trouver isomorphismes sur la cible
(potentiellement partiellement recouvrant). Afin calculer le temps de recherche de $c_n$ nous devons prendre en compte
la quantité de sous isomorphismes précédemment trouvés. Le calcul du temps de recherche de $c_n$ correspondera donc au temps de
calcul de $C_{n-1}$ auquel on vient ajouter le temps de recherche de la dernière extension sur chacun des isomophismes de taille
...
resterait donc trois liaisons à suivre. Nous appélerons cette quantité l'arité restante. On peut donc écrire :
...
chaine. Ce temps est un coût exact de le recherche sur la base d'apprentissage. En normalisant cette valeur par le nombre de
...
produit des filtrages de chaque extension d'une chaine. Les valeurs des filtrages estimés s'obtiennent imédiatement à partir des
comptages dans la base d'apprentissage (Voir \ref{app_compt} pour un exemple). NB : Le taux de filtrage peut être suppérieur à 1
car il est possible d'obtenir plus d'isomophismes après une extension que avant.
...
\paragraph{}Le temps de recherche optimal d'un motif est donc le temps de recherche de la chaine qui minimise le coût. Donc :
...
\paragraph{}Appliquons maintenant cette formule de calcul du temps sur notre exemple pour trouver la meilleure chaine possible par
rapport à la base d'apprentissage. Pour le motif de l'exemple, nous pouvons nommer les différentes chaines possibles de $\alpha$
jusque $\delta$.
...
Naïvement, on peut appliquer la formule sur chacune des chaines possible et trouver la chaine optimale qui minimise la fonction de
coût. Développons l'exemple pour la chaine $c_{\alpha}$.
...
\paragraph{}En appliquant cette formule sur toutes les chaines on peut se rendre compte que certains de nos calculs sont inutiles.
Prenons l'exemple des chaines $c_{\alpha}$ et $c_{\beta}$. Le dernier calcul de la chaine $c_{\beta}$ est inutile. Lors
du calcul des deux chaines, on aurait pu se rendre compte que l'on pouvais déjà décider de quelle chaine était la meilleure à
partir du résultat de la sous chaine $a-b$. Ceci nous amène à transformer le calcul sous forme de programmation dynamique dont les
...
\begin{figure}
  ...
  \caption{\label{markov}En vert le chemin prit par l'algorithme markovien pour générer la chaine du motif a-b-d}
\end{figure}

\paragraph{}Cette méthode exacte nous permet de garantir la qualité de nos chaines sur la base d'apprentissage mais cela se paye
en temps de calcul. En effet, même par programmation dynamique le temps de précalcul des chaines est très élevé si les motifs sont
...
labels soit différent (C'est ce que nous avons esayer de faire pour simplifier la recherche d'isomophismes).

Une chaine (au sens ligne et pas chaine comme défini ici) représente le motif le plus simple. Pour une chaine de noeuds de taille 
...
calculer $n-1$ parmis $n$ valeurs puis $n-2$ parmis $n$, ... Dans ce cas, la complexité sera de l'ordre de $2^n$.
...
\paragraph{}Dans un exemple précédent (TODO : ref), nous avons montré que le début d'une chaine est très critique pour le temps
de recherche de celle-ci. L'idée est donc de générer la chaine par un mélange des deux techniques précédentes. Les débuts de la 
chaine seront être recherchés en exact jusqu'à une taille $k$ définie à l'avance puis le reste des extensions se fonront en
utilisant l'algorithme glouton. Cette découpe des chaines en deux parties nous permet de garantir une recherche très rapide d'un
motif sur la base d'apprentissage tout en limitant fortement le temps de précalcul nécessaire pour la construiction de l'index.
...
\paragraph{}L'algorithme et les indexs présentés dans la section précédente permettent de retrouver rapidement toutes les
occurences d'un motif au sein d'une molécule cible. Jusqu'à présent nous avons laissé supposé que dans notre cas
...
un monomère en entier au sein d'un polymère. Lorsques les monomères sont liés ensemble pour former le polymère, chacun perd une
partie de ses atomes dans ces liaisons. Nous appélerons les molécules incluses des résidus (les monomères sans leurs atomes de
liaison).

\paragraph{}Prenons l'exemple de la Cystéine pour bien comprendre. La cystéine est présente dans 47 peptides de la base de données
...
inversée. Au total la cystéine présente dans ce peptide a donc perdu trois atomes sur deux sites distincts. Das d'autre cas
...
d'hydogéne pour se lier avec un autre atome de soufre et ainsi réaliser un pont disulfure. Sur ce simple exemple on remarque que
...
différentes. Il est donc nécessaire de connaitre les différents résidus possibles pour chaque monomère afin de pouvoir les
rechercher en utilisant l'algorithme de recherche exact décrit précédemment.

\paragraph{Figure non ordonnée de quelques résidus de cystéine}
...
regardant précisément les liaisons qui apparaissent pour un type de polymère, nous pouvons créer une liste de régles de perte
...
cetaines règles comme celles du pont disulfure décrite précédemment, ne s'applique qu'en présence d'une autre liaison. Il n'est
...
\begin{figure}
  ...
  \caption{\label{sulfure}Représentation de la famille de résidus de la cystéine}
\end{figure}
...
Nous appélerons \textbf{famille} cette représentation des résidus sous forme de GOA.

\paragraph{}Le noeud le plus éloigné du monomère sera le noeud
qui aura subit le plus de modification. Nous appélerons le résidu de ce noeud le \textbf{résidu racine}. Ce résidu à un intéret
...
recalculer complétement un isomorphisme une fois que la racine trouvée. Pour passer d'un résidu $r$ à l'un de ses parents $p$ il
suffit de rechercher à étendre l'isomophisme de $r$ avec les atomes que nous avions considérés perdus pour passer de $p$ à $r$
...
\begin{algorithm}[H]
  ...
  isomophisme du résidu avec une sous partie du polymère.}
  ...
  Soit $P$ un ensemble de résidus à processersser initialisé avec les parents de $r_r$\;
...
\paragraph{}La méthode de recherche des isomophismes de sous graphe présentée précédemment, nous permet de trouver
toutes les occurences de chacun des monomères dans un peptide cible.

Puisque nous connaissons les atomes contenus dans chacun des isomophismes découverts, nous pouvons également connaitre chacun des
isomophismes découvert par atome.
L'étape de pavage consiste à choisir un sous ensemble des isomophismes trouvés, de manière à maximiser les atomes couverts par ces
isomophismes en ayant pour chaque atome au plus un seul isomophisme présent dans ce sous ensemble.
...
permettent pas forcément de reconstituer un puzzle complet. De plus, parmis les pièces que nous disposons, certaines se
chevauchent. La phase de pavage consiste donc à choisir parmis les pièces celles que l'on souhaite garder en maximisant le
remplissage de notre puzzle et sans avoir de pièces chevauchantes.

\paragraph{Schéma : représentation symbolique du pavage}
...
pièces trouvées. Dans le cas contraire, on ne peut pas reconstruire complétement le peptide à partir des résidus trouvés. 
Dans ce cas, notre
...
\paragraph{Schéma : Les différentes types de reconstructions possibles}
...
problème représenté par un ensemble de variables, une fonction de score ainsi qu'une liste de contraites sur les variables. La
...
représentées par le fait que la somme de toutes les variables des résidus présents sur un atome ne peut être suppérieure à 1.
...
la liste trièe en ajoutant le résidu courant à la solution uniquement si il ne chevauche aucun autre résidu de la solution.
...
\begin{algorithm}[H]
  ...
  \KwResult{Un sous-ensemble de $RES$ de pièces non-chevauchantes séléctionnées par l'algorithme glouton.}
...
C'est un critère classiquement utilisé pour la résolution gloutone du problème du sac à dos\cite{_probleme_2016}.

\paragraph{Schéma : ordres de tri}
...
une valeur pour chacune de ces règles. Cette valeur reprèsente le poids que l'utilisateur souhaite donner à chacune des liaisons
...
Sur certaines données, alors que la phase d'isomophisme a identifié tous les monomers qui sont effectivement présents dans le
...
obtenir la solution avec les isomophismes déjà effectués.
...
pour les remettre ne serait pas productif) puis nous essayons de compléter la solution en relancant un pavage.
...
\begin{algorithm}[H]
  ...
  \KwResult{$SOL$, un sous-ensemble de $RES$ de pièces non-chevauchantes séléctionnées par la modulation}
...
posible après $i$ appels récursifs ($i$ paramétrable).
...
découpés correctement en utilisant les algorithmes précédents. Il y a essentiellement deux cas pour lesquels l'isomophisme
...
proton ait été perdu et lors de la phase d'isomophisme, la fonction de matching de labels ne répond plus correctement car il
manque un décorateur $H$ sur l'un des atomes. Dans le second cas c'est encore l'isomorphisme qui ne reconnait pas un résidu car
celui-ci est intégré sous une forme tautomérique (voir figure \ref{tautomer}).
Un tautomère de molècule est un isomère structurel de cette molécule.
C'est à dire que la molécule tautomérique contient l'ensemble des atomes de la molécule initiale  mais certains
de ces atomes ne sont plus liés ax même voisins. Dans le cas d'un tautomère, c'est un noyau d'hydrogène qui se déplace et change
...
Dans les deux cas seuls les ``décorateurs'' des labels sont modifiés. Le moyen simple de reconnaitre ce type de molécule est alors
...
La pluspart des monomères NRP ne contiennent que des composés C N O et H.
Ne pouvant plus utliser les H comme décorateurs et en indifférenciant les liens, le nombre de labels différents chute à 9.
Il est donc compréhensible que le temps de calcul augmente beaucoup.

\paragraph{}La solution est une nouvelle fois hybride.
Dans un premier temps, nous cherchons rapidement une solution en effetuant un
isomophisme contenant tous les labels (isomorphisme {\em strict}), puis en effectuant un pavage et une modulation.
Dans un second temps si aucune solution complète n'est trouvée, nous relancons tout le procéssus en commençant par
une nouvelle phase d'isomorphisme sans les décorateurs (isomophisme {\em light}).
...
Le première partie du schéma (au dessus des pointillés) est composée des deux phases de précalcul.
Cette étape de précalcul est lourde en temps mais n'est à effectuer qu'une seule fois.
...
La structure en boucle avec l'isomorphisme light nous permet pour la pluspart des cas de sortir très vite de l'algorithme.
