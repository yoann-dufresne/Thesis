...
Le but est de requéter une base de données de moléules en utilisant des expressions régulières représentant des sous-molécules d'intérêt.
...
Les auteurs pensent qu'il est plus utile de converver les formats atomiques en base plutôt que les séquences protéiques.
...
\begin{figure}[!h]
  \begin{center}
    ...
    Sous l'annotation peptidique est écrite la formile SMILES après concaténation de tous les SMILES des monomères trouvés.
    Schémas issus de la publication}
  \end{center}
\end{figure}

\paragraph{}Pour mener à bien chaque ST, les auteurs ont besoin d'une table de transformation qui ressence les structures atomiques de chacun des monomères à transformer.
...
En effet, si les monomères ne possédent pas de groupement carboxyl et amine, la reconstruction ne peut pas être effectuée sans choix aléatoires d'orientation.
...
\begin{figure}[!h]
  \begin{center}
    ...
    A droite sont répertoriées les deux ponts disulfures extrait hors de la chaine principale.
...
Le premier temps représente la phase de recherche des monomères et le second temps la convertion des moomères trouvés en une annotation.
L'idée de la première phase est de rechercher chaque monomère connu en masquant les atomes déjà utilisés par des monomères précédamment trouvés.
...
Dans le cas des NRP, certains monomères ne possédent pas les deux domaines pour les liaisons peptidiques.
...
Dans ce cas, vu qu'aucun retrour en arrière n'est effectué, l'annotation sera innexacte voir incomplète.


\subsubsection{molBLOCKS, une méthode destructive}

\paragraph{}Le logiciel momBLOCKS n'est à la base pas du tout concu pour le travail que nous cherchons à effectuer.
...
Pour cela le logiciel découpe les polymères en petits morceaux et fait des comptages sur ces morceaux.
Pour espérer découvrir des monomères réalistes, les découpes ne sont pas faites au hasard mais selon des règles exprimées par l'utilisateur.
Cependant, il n'est jamais exclu que les règles définies par l'utilisateur ne sur-découpent pas les molécules.

Dans notre cas, nous pourrions utiliser un tel logiciel pour découper les peptides d'intérêt puis effectuer par la suite une étape de reconnaissance des monomères générés.
Cependant, aux vues des liaisons particulières de certains NRP, il est fort probable que certains morceaux ne ressemblent jamais à un monomère NRP connu.
Le fait de ne pas garantir la véracité des monomères dès la découpe est un fort unconvénient dans notre cas.
