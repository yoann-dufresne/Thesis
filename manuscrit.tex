...
\paragraph{TODO : schéma MCS}

\paragraph{}Commençons par définir précisément le problème du MCS.
Comme le nom l'indique, le problème du MCS est un problème de recherche de la sous partie commune maximale entre deux graphes (Voir figure TODO).
...
Le problème du MCS à été prouvé NP-Complet\cite{garey_computers_1979}, ce qui empèche (en tout cas pour le moment) l'émergence d'algorithmes de résolution polynomiaux efficaces.
...
Un graphe de compatibilité (GC) est un graphé créé à partir du croisement des deux graphes (qu'on appélera G et H).
...
Encore une fois, certaines des techniques citées permettent des accélérations pratiques en coupant des branches de recherhce mais jamais l'ordre de grandeur de la complexité n'est changé.
...
Lors d'une requête, le motif recherché est à son tour tranformé en fingerprint et une mesure de similarité est effectuée avec les vecteurs présents en base.
...
Un nombre fixe de bijections est maintenu en permanance.
...
C'est à dire qu'un graphe G est sous graphe d'un graphe H si tous les noeuds et arétes de G doivent être retrouvés dans H structurés de la même manière pour dire que G et un SI de H.
...
Certaines de ses variantes sont toujours utilisées dans certaies librairies telles que Openbabel~\cite{oboyle_open_2011}.
...
Une case de la matrice comporte un 1 si les labels des noeuds de la case sont compatibles (labels) et que le degré du noeuds de H est suppérieur ou égal au degré du noeud de G.
...
Au sein de cette ligne l'algorithme choisi une colonne contenant un 1 et considère que c'est le binome des noeuds ligne/colonne qui fait partie de l'isomorphisme.
...
Sinon, il suffit de continuer récurssivement jusqu'à obtenir un unique 1 par ligne.
...
\paragraph{}Pour pouvoir rechercher les monomères nous devions choisir entre toutes les appoches algorithmiques citées ci-dessus.
...
Pour les entrées de s2m, nous constituons une base de monomères pour chaqun des jeux de données.
...
\begin{table}[!h]
  ...
  \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    \hline
    & \multicolumn{4}{c|}{Heurisitique} & \multicolumn{4}{c|}{MIP} \\
    \hline
  ...
  Dans la première colonne se trouvent les binomes des valeurs des paramètres retry et modulation}
\end{table}
...
\paragraph{}En comparant les résultats des deux algorithmes, on s'apperçoit que l'algorithme comprenant l'heuristique de pavage obtient de meilleurs résultats (pour une modulation $\ge$ 2) que l'algorithme incluant un pavage exact.
