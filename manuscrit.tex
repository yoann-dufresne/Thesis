...
\begin{figure}[!ht]
  \begin{center}
    ...
    Le MCS obtenu n'est pas connexe du fait des liens vers le noeud rouge, qui ne sont pas les mêmes dans G et H.}
  \end{center}
\end{figure}
...
Une clique est un ensemble de noeuds dans un graphe tel que tous les noeuds de cet ensemble sont reliés entre eux.
...
Un MCS est une bijection entre deux sous ensembles de noeuds dans deux graphes distincts.
Les méthodes de backtrack effectuent une reconstruction itérative de cette bijection.
Les couples de noeuds compatibles, n'étant pas encore présents au sein de la bijection, sont ajoutés un à un.
...
Le couple ligne/colonne nous donne le couple noeud de G/noeud de H a peut être ajouter à l'isomorphisme.
...
Les SI se forment par ajouts successifs de couples de noeuds compatibles avec retour en arrière s'il n'est plus possible d'avoir un couple.
...
Secondement, le MCS ne paraît pas très pratique dans notre cas.
...
isomorphismes de graphes ou de sous-graphes non labellés d'un graphe $G$ sur un graphe $H$.
...
On voit également que la recherche est d'autant plus rapide que l'on choisit prioritairement les noeuds avec des labels de faible
fréquence.

Là où l'algorithme général VF2 était en $O(n^m)$, la complexité est ici réduite dans le pire des cas à $O(k^n)$ où $k$ est le
comptage du nombre de noeuds du label le moins filtrant.
La taille de l'arbre de recherche est diminuée par la sélectivité des labels de noeuds.
...
trois labels variants depuis l'ancien label unique $N$ qui était accompagné de voisins hydrogènes. Les labels sont plus nombreux et leur fréquence à la fois moins élevée et plus homogène, ce qui les rend plus sélectifs.
...
\paragraph{}Globalement, plus le nombre de labels est élevé et leurs fréquences homogènes, et moins l'algorithme VF2 aura de
...
d'arêtes (une clique entre tous les nouveaux noeuds représentant les anciennes arête de cet ancien noeud). Nous étendons le 
...
gros avantages de cette représentation est d'inclure directement dans les labels des noeuds ce qui touche à l'arité entre noeuds.
La fonction
...
vient tout de suite en tête est de choisir un à un les noeuds que l'on souhaite ajouter au motif en choisissant toujours celui
...
labels nécessaire sont également présents sur cette même figure. La chaîne qui sera générée commence forcément par le noeud
...
noeuds de la base, nous pouvons obtenir le coût de recherche par noeud et ainsi pouvoir estimer le temps de calcul quelle que soit
...
La redondance des labels permet quant à elle de couper certaines branches lors de l'indexation et ainsi de diminuer un peu
...
Il n'y a aucun doute sur le fait qu'elle continura a évoluer rapidement.
...
Nous avons profité de la création du script de mises à jour pour revoir complétement notre facçon de stocker les taxons.
...
Pour le moment il existe les status ``curated'' et ``putative'' associé à chauqe peptide.
...
Cela nous facilite la tâche en nous assurant du status des molécules que nous ajoutons à Norine.
...
En regardant rapidement, on s'apperçoit que de nombreuses molécules sont constituées de seulement un ou deux monomères.
Ajoutons un filtre de nombre de monomères minimal.
Dans les statistique de Norine, la plupart des peptides possédent au moins 7 monomères.
...
Rappellons que les protéines classiques sont constituées uniquement de 20 acides aminiés différents.
...
Nous ajoutons comme filtre l'impossibilité d'être linéaire (nombre de liaisons suppérieurs ou égal à nombre de noeuds) et l'obligation de posséder au moins deux monomères ne faisant pas partie des 20 de base.
Finalement nous obtenons 1097 peptides.

\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=450px]{Figures/contributions/didemnin_B.png}
    \caption{\label{didemin}À gauche une des sorties après processus de filtration et à droite le NRP corespondant dans la base Norine.}
  \end{center}
\end{figure}
