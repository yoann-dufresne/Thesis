...
l'incorporation de plusieurs centaines de {\em monomères} différents (briques de bases).
La base de données de référence des NRP en compte pour le moment 533.
Les monomères peuvent provenir de différentes classes.
Parmis eux, on compte naturellement les 20 acides aminés standards ainsi qu'un grand nombre de dérivés proches.
...
Ce genre monomères, permet lorsqu'ils sont inclus, d'obtenir des structures à embranchements ou cycles (Voir figure TODO).
...
\paragraph{}Pour compléter la gamme d'éléments permettant la diversité des compositions/structures, d'autres liaisons que les liaisons peptidiques sont réalisées entre les composants des NRP.
De la même manière que pour les protéines classiques, les NRP peuvent être assemblés via des liaisons amino-acide entre monomères.
La plupart des monomères inclus effectuent d'ailleurs leurs liaisons de cette manière.
Nous pouvons ensuite lister quatre types de liaisons différentes.

\paragraph{}La première est effectuée par les monomères contenant un atome de soufre.
Comme dans des protéines classiques les monomères souffrés peuvent effectuer des ponts dissulfure.
Cependant ce type de liaison n'est pas la seule impliquant un soufre.
Comme nous le verrons plus tard lors de la description des modules NRPS Cy, l'atome de soufre peut également intervenir dans une cyclisation entre deux monomères.

\paragraph{TODO : cycle Cys}

\paragraph{}La seconde possibilité est d'effectuer des liaison sur des cylces aromatiques présents au sein des monomères (cycles Imidazoles et Benzene).
Dans les deux cas c'est un atome d'hydrogène qui est arraché au cycle afin de pouvoir y lier un monomère.
Dans le cas du cycle Imidazole l'hydrogène est toujours arraché à un des deux atomes C voisins dans le cycle.
...
\paragraph{TODO : a réécrire/vérifier}Enfin, nous avons regroupé derrière le nom ``liaison hydroxyl'' les liaisons s'effectuant entre des groupements OH qui ne perdent que leur H lors de la recombinaison.

\paragraph{}L'ensemble des liaisons présentées ici permettent d'augmenter le nombre de monomères candidats aux inclusions et le nombre de site de liaison sur chaque monomère.
Ceci permet donc d'augmenter grandement le nombre de peptides non-linéaires et ne contenant pas de monomères standards.
