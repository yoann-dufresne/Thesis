...
\paragraph{}Afin de bien comprendre la voie de synthèse non ribosomique, commençons par quelques rappels rapide sur la synthèse
des protéïnes classiques. Les protéines sont assemblées dans la cellule par un organite appelé ribosome. Les ARN messagers sont
les vecteurs de l'information protéique. Ils sont la réplication d'un morceau d'ADN qui sera traduit par triplets de nucléotides
en chaine d'acides aminés. Les 64 codons possible ($4^3$ nucléotides) sont traduit en 20 acides aminés que l'on appélera acides
aminés protéinogènes (justement car ils interviennent dans la synthèse classique de protéines). Ces acides aminés sont tous 
composés d'un même squelette contenant les atomes permettant deux liaisons permettant ainsi la formation de chaines peptidique
ainsi qu'un radical variant selon l'acide aminé (Voir figure TODO). L'un des côté du squelette contient un groupement amine
($NH_2$) et l'autre un groupement carboxyle ($C(=O)OH$). Ces deux groupements se lient entre eux en chaine et créent ainsi la
protéine.

\paragraph{Figure acides aminés}

\paragraph{}Une protéine assemblée se replie sur elle même et c'est cette forme repliée qui détermine son activité. Plus
précisément, ce sont les proprités des éléments en contact avec l'extérieur de la protéine (les atomes qui ne se retrouvent pas
enfermé au milieu du repliement) qui déterminent l'activité de celle-ci. Ces propriétés sont donc très dépendantes du repliement
et des types de radicaux exposés.



\subsection{Généralités sur les peptides non ribosomiques}
%TODO : Qui sont les producteurs ?
\paragraph{}Tout comme les protéines classiques, les peptides non ribosomiques (Non Ribosomal Peptide (NRP) en anglais) sont des
des molécules résultant d'assemblages de briques de base. Cependant, comme le nom l'indique, la voie de synthèse d'un NRP est
différente de celle des protéines. Cette voie de synthèse est constituée d'une étape supplémentaire. Comme nous venons de le
présenter, lors d'une création classique de protéine, l'ADN est transcrit en ARN et lui même est traduit en protéine. Ici, la
protéine produite n'est pas le produit final mais une enzyme qui peut agir seule ou se grouper en complexe afin d'assembler
des polymères. Ces complexes sont appelés des synthétases (Non Ribosomal Peptide Synthetase (NRPS)). Là où les protéines
classiques sont composées des 20 acides aminés standards, la synthèse non ribosomique permet
l'incorporation de plusieurs centaines de monomères (briques de bases) différents.

\paragraph{TODO : schéma Dpr}

\paragraph{}Les NRPS sont également plus souples dans la façon d'incorporer des monomères. Certains monomères possédent plus de
deux groupements capables de se lier. Ainsi sur l'exemple (TODO : Dpr), on peut constater que le monomère nommé Drp possède en
plus des deux groupements de liaison peptidique, un second groupement amine en bout chaine latérale. Ce genre monomère
potentiellement lié trois fois, permet d'obtenir des formes branchées ou cyclisées et ainsi de ne plus se restreindre à un
polymère linéaire.

\paragraph{TODO : Liste exhaustive de NRPs}

\paragraph{}Cette posssibilité de structures branchées permet de contraidre les repliements et de créer de nouvelles structures
externes. La composition en monomère rentre également en compte dans l'activité finale des peptides. Les structures chimiques des
nombreux monomères inclus par les NRPS permettent à leur tour la création de surfaces externes différentes de celles produites
par les 20 acides aminés de base. Grâce à ces deux caractéristiques, les organimes producteurs ont accès à de nombreuses activités
peptides impossibles à obtenir depuis des protéines classiques.

% Grande diversité d'activités
% Intérêt fort pour ces activités
% Se comprend par l'énergie dépenssée par la cellule



\subsection{Les synthétases}

\subsubsection{Des enzymes modulaires}

\subsubsection{La spécificité des modules A}

\subsubsection{Inclusion monomériques par les modules C}

\subsubsection{Les modules secondaires}



\subsection{L'activité finale d'un peptide}

\subsubsection{Formes et compositions}

\subsubsection{Modifications post-synthèse}
