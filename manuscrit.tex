...
Ainsi sur l'exemple (TODO : Dpr), on peut constater que le monomère nommé Drp possède un groupement amine supplémentaire à celui déjà présent dans le squelette des acides aminés classiques.
...
\paragraph{}Cette possibilité de structures branchées permet de contraindre les repliements et de créer de nouvelles structures
...
par les 20 acides aminés de base. Grâce à ces deux caractéristiques, les organismes producteurs ont accès à de nombreuses activités
...
Les synthétases sont organisées en modules consécutifs qui permettent chacun de capturer modifier et intégrer un monomère au NRP.
...
\paragraph{Figure hc-toxin}

\paragraph{}Prenons pour exemple le NRP appelé hc-toxin [Ref mibig].
...
Le découpage pour ce NRP est représenté sur la figure [REF figure].
...
\paragraph{}En 1999, Stachelhaus et al. publient un article analysant la structure de nombreux domaines A.
...
\paragraph{}Depuis 2011, l'outil NRPSPredictor2 est venu affiner la reconnaissance de la spécificité.
Les auteurs ont entrainés un Modèle de Markov Caché (Hidden Markov Model (HMM)) sur des centaines séquences de domaines A connus.
Cet outil prédit un avec différents niveaux de granularité les monomères inclus à partir des domaines A.
%Chaque monomère est regroupé avec d'autres dans des clusters par affinités.
%Chacun de ces clusters est alors regroupé avec d'autres sur 3 niveaux de granularités. (TODO schéma de l'article ?)
Les monomères sont identifiés avec une moyenne précision-rappel (F-Mesure) de 80\%.
L'outil prédit également avec une très grande précision (F-mesure > 95\%) le type du monomère inclus (polaire, hydrophobique, aliphatique, ...).
Cet outil est particulièrement précieux pour la découverte et l'annotation de domaines A de NRPS.
...
\paragraph{}Le but du domaine T est d'assurer la logistique au sein du module dans lequel il est contenu.
Ce domaine est également souvent appelé "Peptidyl Carrier domains".
...
\paragraph{}Le domaine C est par convention considéré comme le premier domaine de chaque module.
...
Bien que sa fonction principale reste toujours la même, il existe plusieurs variants de ce domaine.
...
Ces deux configuration sont en fait des images miroirs l'une de l'autre [TODO : voir figure].
On appel ces deux configurations Leavus (L) et Dexter (D).

\paragraph{Schéma D-L}
...
Il arrive parfois que cette transformation du monomère ne soit pas portée par un module E mais directement incluse dans le domaine C précédent.
Ce domaine est alors appelé domaine C/E et assure les deux fonctionnalités.

\paragraph{Schéma de la transformation}
...
Ce module n'existe à priori que pour les modules d'initiation.
Il permet la formylation (ajout d'un groupe C(=O)H) sur le groupement amine du premier monomère.
