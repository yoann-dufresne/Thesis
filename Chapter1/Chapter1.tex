...
Lorsque la chaîne n'est constituée que de quelques acides aminés (généralement moins de 25) on ne la nomme plus protéine mais peptide.
...
\begin{figure}[h!]
  \begin{center}
    ...
    De gauche à droite : deux représentants des acides aminés classiques ; plusieurs dérivés d'acides aminés ; un acide gras ; deux sucres.}
  \end{center}
\end{figure}
...
\begin{figure}[h!]
  \begin{center}
    ...
    Au sein de l'Edeine A, seules deux liaisons classiques du squelette peptidique sont effectuées entre le Dpr (en rose) et les autres monomères.
...
Comme pour les protéines classiques, les monomères soufrés peuvent effectuer des ponts disulfures (liaison entre deux atomes de soufre en perdant deux atomes d'hydrogène).
...
\begin{figure}[h!]
  \begin{center}
    ...
    Les ovales bleu ciel représentent les domaines T.
...
La séquence d'acides aminés de ce type de domaine varie fortement d'un domaine A à l'autre et seuls 16\% de la séquence du domaine sont conservés (sur environ 200 acides aminés codant un domaine)~\cite{stachelhaus_specificity-conferring_1999}.
...
Il existe également des molécules très proches des NRP appelés polykétides qui peuvent venir s'hybrider avec des NRP (voir section \ref{PKS_p}).
...
Pour les NRPS, l'annotation correspond à la phase de recherche de gènes NRPS dans le génome ainsi qu'à l'identification des différentes briques contenues dans ces gènes (entre autres les modules).
...
Pour cela, il faut tout d'abord constituer une petite base de données de domaines NRPS très généralistes puis les rechercher dans le génome cible à l'aide d'algorithmes d'alignement comme BLAST.
...
Le succès a été tel que les auteurs ont dû recourir à des listes d'attente avec priorité pour pouvoir obtenir des résultats depuis le serveur dédié.
...
C'est notamment le cas des logiciels NRPQuest~\cite{mohimani_nrpquest:_2014} et ``Genomes-to-Natural Products platform'' (GNP)~\cite{johnston_automated_2015}.
